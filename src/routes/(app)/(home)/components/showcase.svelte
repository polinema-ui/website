...
<section aria-label="Framework Showcase" class="relative w-full overflow-hidden bg-[#0a0d13] px-6 py-24 sm:py-32">
	<div
		class="absolute top-0 left-1/2 -z-10 h-200 w-300 -translate-x-1/2 bg-[linear-gradient(ellipse_at_top,var(--tw-gradient-stops))] from-blue-500/20 via-transparent to-transparent"
		aria-hidden="true"
	></div>
	<main class="mx-auto flex max-w-6xl flex-col items-center gap-14">
		<header class="flex max-w-3xl flex-col gap-5 text-center">
			<h2 class="font-sans text-4xl font-extrabold tracking-tight text-white sm:text-[44px]">
				Satu Desain, <span class="text-blue-400">Multi Ekosistem</span>
			</h2>
			<p class="font-sans text-lg leading-relaxed text-neutral-300 sm:text-[19px]">
		...
		<article
			class="grid w-full grid-cols-1 overflow-hidden rounded-xl border border-white/10 bg-neutral-900 shadow-xl shadow-black/40 lg:grid-cols-12"
		>
			<nav
				aria-label="Framework list"
				class="flex flex-col gap-1 border-b border-dashed border-white/10 p-4 sm:p-6 lg:col-span-4 lg:border-r lg:border-b-0 xl:col-span-3"
			>
				...
				{/each}
				<p class="mt-4 rounded-lg bg-blue-500/10 px-3 py-3 text-xs leading-relaxed text-neutral-300">
			...
			<section class="flex flex-col gap-6 p-6 sm:p-8 lg:col-span-8 xl:col-span-9">
				<div>
					<div class="flex items-center gap-3">
						...
						<h3 class="font-sans text-xl font-bold tracking-tight text-white">
							{currentData.label}
						</h3>
					</div>
					<p class="mt-3 font-sans text-[15px] leading-relaxed text-neutral-300">
						{currentData.description}
					</p>
				</div>

				<figure class="overflow-hidden rounded-xl border border-white/10 bg-neutral-900">
					<figcaption class="sr-only">Code snippet for {currentData.label}</figcaption>
					<header class="flex items-center justify-between bg-white/5 px-4 py-1">
					...
					<div class="bg-neutral-950 pb-2 pl-2">
