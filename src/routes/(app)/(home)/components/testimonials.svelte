...
<section class="relative flex w-full flex-col items-center gap-12 overflow-hidden bg-white py-20 dark:bg-black">
	<header class="flex flex-col items-center gap-3 px-6 text-center">
		<h2 class="font-sans text-3xl font-bold tracking-tight text-neutral-900 sm:text-4xl md:text-[44px] dark:text-white">
			Kata Mereka Tentang Polinema UI
		</h2>
		<p class="max-w-xl font-sans text-base text-neutral-600 sm:text-[17px] dark:text-neutral-300">
				...
				<ul class="flex gap-6">
					{#each [...row, ...row, ...row] as item, itemIndex (`${item.handle}-${rowIndex}-${itemIndex}`)}
						<li class="min-w-0 flex-[0_0_85%] sm:flex-[0_0_48%] md:flex-[0_0_31%]">
							<Card
								class="h-full rounded-2xl border border-neutral-200 bg-white p-6 shadow-[0_4px_20px_rgba(0,0,0,0.03)] transition-all hover:border-blue-500/50 hover:shadow-md dark:border-white/10 dark:bg-black"
							>
									...
									<header class="flex items-center justify-between">
										<figure class="flex items-center gap-3">
											...
											<figcaption class="flex flex-col text-left">
												<span class="font-sans text-sm font-semibold text-neutral-900 dark:text-white">{item.name}</span
												>
												<span class="font-sans text-xs text-neutral-500 dark:text-neutral-500">{item.handle}</span>
											</figcaption>
										</figure>
										<HugeiconsIcon icon={NewTwitterIcon} size={16} class="text-neutral-400" />
									</header>
									<blockquote
										class="mt-4 text-left font-sans text-sm leading-relaxed text-neutral-600 dark:text-neutral-300"
									>
										"{item.content}"
									</blockquote>
								</article>
